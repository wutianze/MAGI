...
def startProcs(subsystems,path_to_cgroup,cmd):
    if subprocess.getstatusoutput("cgexec -g " + subsystems + ":/" + path_to_cgroup + " " + cmd)[0] != 0:
        print("Err: Start Process in cgroup Fail")


def cpu_quotaSet(group,quota):
    if subprocess.getstatusoutput("cgset -r cpu.cfs_quota_us=" + str(quota) + " " + group)[0] != 0:
        print("Err: quotaSet Fail")
        return -1
    return 0

def cfs_periodSet(group,period):
    if subprocess.getstatusoutput("cgset -r cpu.cfs_period_us=" + str(period) + " " + group)[0] != 0:
        print("Err: cpu_periodSet Fail")
        return -1
    return 0
    
def cpusSet(value,path_to_cgroup):
    if subprocess.getstatusoutput("cgset -r cpuset.cpus=" + str(value) + path_to_cgroup)[0] != 0:
